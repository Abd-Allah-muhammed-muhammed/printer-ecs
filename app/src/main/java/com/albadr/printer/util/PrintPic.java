...
public class PrintPic {
    ...
    public byte[] printDraw() {
        ...
        // Print raster bitmap instruction
        imgbuf[0] = 29;// Hex 0x1D
        imgbuf[1] = 118;// Hexadecimal 0x76
        imgbuf[2] = 48;// 30
        imgbuf[3] = 0;// Bitmap mode 0,1,2,3
        // It indicates the number of bytes in the horizontal direction bitmap（xL+xH × 256）
        imgbuf[4] = (byte) (this.width / 8);
        imgbuf[5] = 0;
        // A vertical direction bitmap Points（ yL+ yH × 256）
        imgbuf[6] = (byte) (getLength() % 256);//
        imgbuf[7] = (byte) (getLength() / 256);

        s = 7;
        for (int i = 0; i < getLength(); i++) {// Height cycle bitmaps
            for (int k = 0; k < this.width / 8; k++) {// Width cycle bitmaps
                int c0 = nbm.getPixel(k * 8 + 0, i);// Returns the specified color coordinates
                int p0;
                if (c0 == -1)// 判断颜色是不是白色
                    p0 = 0;// 0,不打印该点
                else {
